...
def load_data(session_id, date):
    file_path = os.path.join(DATA_DIR, f"{session_id}_{date}.pkl")
...
def save_data(session_id, date, data):
    file_path = os.path.join(DATA_DIR, f"{session_id}_{date}.pkl")
...
def get_initial_state(session_id, date):
    return load_data(session_id, date)

# Function to save whiteboard as image file
def save_whiteboard(image_data, session_id, date):
    img = Image.fromarray((image_data * 255).astype(np.uint8))
    img.save(os.path.join(DATA_DIR, f"{session_id}_{date}_whiteboard.png"))

# Function to load whiteboard from image file
def load_whiteboard(session_id, date):
    file_path = os.path.join(DATA_DIR, f"{session_id}_{date}_whiteboard.png")
    try:
...
def save_current_state():
    ...
    save_data(session_id, st.session_state.date, data)
    if st.session_state.whiteboard is not None:
        save_whiteboard(st.session_state.whiteboard, session_id, st.session_state.date)

# Function to plan for tomorrow
def plan_tomorrow():
    tomorrow_date = datetime.now() + timedelta(days=1)
    st.session_state.date = tomorrow_date.strftime('%Y-%m-%d')
    initial_state = get_initial_state(get_session_id(), st.session_state.date)
    st.session_state.tasks = initial_state["tasks"]
    st.session_state.notes = initial_state["notes"]
    st.session_state.whiteboard = load_whiteboard(get_session_id(), st.session_state.date)
...
if 'tasks' not in st.session_state:
    session_id = get_session_id()
    initial_state = get_initial_state(session_id, st.session_state.date)
    st.session_state.tasks = initial_state["tasks"]
    st.session_state.notes = initial_state["notes"]
    st.session_state.whiteboard = load_whiteboard(session_id, st.session_state.date)

# Sidebar for selecting date
st.sidebar.title("Select Date")
session_id = get_session_id()
data_files = [f for f in os.listdir(DATA_DIR) if f.startswith(session_id)]
